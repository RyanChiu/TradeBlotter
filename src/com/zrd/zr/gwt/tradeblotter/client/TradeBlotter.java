...
import com.zrd.zr.gwt.tradeblotter.shared.InstantMessagingCometSerializer;
...
public class TradeBlotter implements EntryPoint {
	 ...
	 * mCometClient
	 */
	private CometClient mCometClient;
	...
	public void onModuleLoad() {
		...
		CometListener cometListener = new CometListener() {
			...
			public void onHeartbeat() {
				// TODO Auto-generated method stub
				String text = successfulAnchor.getText();
				if (text.length() >= 4) {
					text = ">";
				} else {
					text += ">";
				}
				successfulAnchor.setText(text);
			}
			...
			public void onMessage(List<? extends Serializable> messages) {
				...
				for (Serializable message: messages) {
					if (message.getClass().equals(StatsStruc.class)) {
						StatsStruc stats = (StatsStruc) message;
						totalAnchor.setText("" + stats.total_cases);
					}
				}
			}
			
		};
		CometSerializer cometSerializer = GWT.create(
			InstantMessagingCometSerializer.class
		);
		mCometClient = new CometClient(
